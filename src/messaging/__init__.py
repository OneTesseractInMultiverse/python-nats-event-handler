...
class AbstractNATSSubscriber(object):
    ...
    """
    This class is meant to be extended by classes that need to provide a
    concrete implementation of an Event Subscriber that will subscribe
    to a set of particular events.
    """
    ...
    async def _connection_closed_event_handler(self):
        ...
        :return:
        """
        self._logger.info("Connection to NATS cluster is closed")
    ...
    async def _reconnect_event_handler(self):
        ...
        """
        self._logger.info(
            f"Connected to NATS cluster at: "
            f"{self._nats_client.connected_url.netloc}"
        )
    ...
    async def start(self):
        ...
        if await self._connect():
            self._logger.info(
                f'Connection to NATS cluster was successful. '
                f'Subscribing to {self.subject}'
            )
            self._logger.info(
            ...
            self._logger.error(
                f'Unable to connect to NATS servers at: '
                f'{NATS_SERVER.split(",")}'
            )
